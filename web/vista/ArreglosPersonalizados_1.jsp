                ...
                try {
                    ...
                    const defaultFlowerConfig = {targetSize: 0.7, offsetY: -0.005};
                         ...
                         // 2) Obtener lista de colores
                            const resColores = await fetch('../ControladorFlores?tipoc=mostrar_colores');
                         ...
                         // 3) Configuración base de cada flor
                                    const defaultCfg = { targetSize: 0.7, offsetY: -0.005 };
                                    let options = {};

                                    // 4) Inicializar opciones sin colores
                                    floresData.forEach(item => {
                                      options[item.nombre_flor] = {
                                        modelPath: "../models/" + item.img_3d,
                                        targetSize: defaultCfg.targetSize,
                                        offsetY: defaultCfg.offsetY,
                                        colors: []              // aquí cargaremos los colores
                                      };
                                    });

                                    // 5) Rellenar el array de colors según nombre_flor
                                    coloresData.forEach(c => {
                                      const key = c.nombre_flor;
                                      if (options[key]) {
                                        options[key].colors.push({
                                          name: c.nombre_color,
                                          hex:  c.hex
                                        });
                                      }
